...
public class Float2Type
    extends XmlSimpleType
{

    protected final double[] value;

    public Float2Type(XmlSimpleType attr, String value)
        throws ParseException
    {
        super(attr);
        this.value = Float2Type.parseAndCheckValue(value, this.file);
    }
        ...
        throws ParseException
    {
        ...
        double[] ret = Float2Type.parseValue(tmp, file);
        Float2Type.checkValueBasedFacets(ret);
        return ret;
    }
    ...
    public static void checkValueBasedFacets(double[] value) {
        ...
        if ((value.length < 2)) {
            throw new XmlValidationException(("Value not long enough. Length must be >= 2, but was "+(value.length)));
        }
        if ((value.length > 2)) {
            throw new XmlValidationException(("Value too long. Length must be <= 2, but was "+(value.length)));
        }
    }
