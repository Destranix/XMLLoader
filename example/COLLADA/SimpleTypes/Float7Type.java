    ...
    extends XmlSimpleType
{
        ...
        throws ParseException
    {
        ...
        double[] ret = Float7Type.parseValue(tmp, file);
    ...
    public static void checkValueBasedFacets(double[] value) {
        ...
        if ((value.length < 7)) {
            throw new XmlValidationException(("Value not long enough. Length must be >= 7, but was "+(value.length)));
        }
        if ((value.length > 7)) {
            throw new XmlValidationException(("Value too long. Length must be <= 7, but was "+(value.length)));
        }
    }
