...
public class XmlIDREFS extends XmlSimpleType{
	
	private static final Pattern RESTRICTION_PATTERN = Pattern.compile("(.*(?:(?:[ ]+).*)*)");
	
	protected final String[] value;

	public XmlIDREFS(XmlSimpleType attr, String value) throws ParseException{
		super(attr);
	...
	public static String[]  parseValue(String value, XmlFile file) throws ParseException{
		...
		for(int i=0;i<tmpSplit.length;++i){
			ret[i] = XmlIDREF.parseAndCheckValue(tmpSplit[i], file);
		}
		return ret;
	}
