...
public class XmlListQName extends XmlSimpleType{
	
	private static final Pattern RESTRICTION_PATTERN = Pattern.compile("(.*(?:(?:[ ]+).*)*)");
	
	protected final String[] value;

	public XmlListQName(XmlSimpleType attr, String value) throws ParseException{
		super(attr);
	...
	public static String[]  parseValue(String value){
		...
		/*
		ret = new String[tmpSplit.length];
		for(int i=0;i<tmpSplit.length;++i){
			ret[i] = (String) XmlSimpleType.parseAndCheckValue(tmpSplit[i], file);
		}
		*/
		return tmpSplit;
	}
